...
- Help text and docs MUST make clear that the public authoring family is `system author`, and that the first shipped authoring wedge is `system author charter`.
...
- The first shipped authoring wedge MUST expose exactly:
...
- [ ] `--help` shows `author` between `setup` and `pipeline`, and documents `system author charter` plus `system author charter --from-inputs <path|->`.
...
- [ ] Setup docs and help state that scaffolded setup flows end with `run \`system author charter\`` and ready setup flows end with `system doctor`.
