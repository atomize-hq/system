...
- the verifier derives the complete all-eleven-leaf dependency audit: 48 H2, 22 H3, exactly 84 frozen positional occurrences, `R01`-`R98`, `O01`-`O50`, `N01`-`N11`, and every heading aggregate; each leaf contains its exact trigger-indexed rows, a heading alone activates none, a source-plus-target union of six or more is broad-only, and omitted, broken, overlapping, unjustified, unclassified, or stale-fanout cases fail closed;
...
- mutable control-pack, orchestration, dispatch-template, proof, and handoff guidance names the minimum exact leaf path/anchor set; subject manifests contain only files whose bytes are under review, including changed routing/proof files when applicable, while unchanged contextual authority remains only in `authority_refs` and/or `contracts_and_gates`; complete-catalog review manifests the index, leaves, verifier, and other files actually changed or reviewed rather than unchanged authority by default;
