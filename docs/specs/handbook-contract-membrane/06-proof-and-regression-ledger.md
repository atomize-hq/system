...
| `PG-CATALOG-01` | the frozen HCM-0.4 `05` semantic payload is partitioned into the exact eight-leaf HCM-0.9 topology with byte-identical ordered parity, one canonical owner per frozen top-level section, stable compatibility links for every frozen anchor, explicit leaf selection by future packets/dispatches, and no historical-record/dispatch rewrite | open; redesigned HCM-0.9 planning under review, decomposition not started |
...
## HCM-0.9 contract-catalog mechanical decomposition proof gate
...
- `05-contracts-schemas-and-gates.md` remains at the stable path and becomes discovery/compatibility metadata only; it does not duplicate canonical contract prose, fenced examples, field rules, defaults, validation, or non-goals;
- the eight leaf files, exact contiguous source spans, per-span digests, and every frozen top-level-section assignment exactly match the HCM-0.9 `SPEC.md` and mechanical decomposition inventory; no section is missing, duplicated, split across owners, or reassigned without a reviewed plan change;
- after validating and removing exactly each leaf's H1 plus one following blank line, concatenating leaf payloads in topology order is byte-identical to the frozen baseline bytes after its single H1 plus following blank line;
...
- the verifier proves only mechanical decomposition: baseline/file/body identity, eight exact spans and H1 byte strings, 48 H2/22 H3 ownership, ordered byte reconstruction, anchors, links, fences, parseable examples, historical immutability, manifest membership, and scope; missing/changed/additional H1/scaffold, omitted changed manifest paths, and forbidden runtime/HCM-0.5 paths have fail-closed negative fixtures; it contains no trigger, route-inventory, co-activation, operation-fixture, fanout, semantic-dependency, or inferred-selection logic;
...
- mutable control-pack, orchestration, dispatch-template, proof, and handoff guidance requires future packet/dispatch authors to list exact leaf paths/anchors explicitly, without inferred closure; subject manifests contain only files whose bytes are under review, including changed guidance/proof files when applicable, while unchanged contextual authority remains only in `authority_refs` and/or `contracts_and_gates`; at each review boundary manifest paths equal all changed non-dispatch paths from the execution-start HEAD, with only additive HCM-0.9 review dispatches exempt from the self-referential comparison and schema-validated separately; complete-decomposition review manifests the index, eight leaves, verifier, and other files actually changed or reviewed rather than unchanged authority by default;
...
- archive-boundary checks, all handoff validator modes, deterministic manifest replay, mechanical decomposition/parity checks, scoped diff inspection, `git diff --check`, and staged GitNexus change detection pass;
...
- the reviewed execution commit and its separate mechanical v1.2 parent handoff/ledger closeout both validate without claiming semantic completion beyond mechanical decomposition.
