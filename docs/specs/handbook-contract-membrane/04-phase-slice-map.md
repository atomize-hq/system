...
| `HCM-0.9` | Correct the contract-catalog topology without changing frozen meaning | stable `05` routing index; focused canonical leaf documents; exact section-to-leaf map; selective orchestration/dispatch/proof/handoff routing; mechanical parity evidence | the ordered canonical leaf payload is byte-identical to the HCM-0.4 `05` body at `214a5b8eb182fce74478df49d4f55d226d65fdf5`; every frozen heading maps exactly once and its legacy `05` anchor routes to the owning leaf; mutable refs select exact leaves; immutable history is unchanged; all structural, routing, scope, and fresh-review gates pass |
...
`HCM-0.9` is a corrective control-maintenance insertion. It depends on the completed HCM-0.4 frozen contract baseline and is sequenced immediately after HCM-0.4 and before HCM-0.5. HCM-0.5 must select its contract/evidence/verdict/gate/dock authority from the HCM-0.9 leaf topology rather than extend the monolith.
...
**Primary outputs:** the stable `05` routing index; the eleven canonical leaf files and exact mapping frozen in [`slices/HCM-0.9/SPEC.md`](slices/HCM-0.9/SPEC.md); a slice-local mechanical parity/routing verifier; updated mutable control-pack, orchestration, dispatch-template, proof, and handoff-routing references; and review/proof evidence. Existing handoff records and dispatches are inputs only and remain byte-immutable.

**Exact exit gate:** all HCM-0.9 acceptance criteria and the HCM-0.9 gate in `06-proof-and-regression-ledger.md` pass; no semantic payload byte from the frozen baseline is added, removed, reordered, or changed; every frozen `##` section has exactly one leaf owner; every frozen `##`/`###` anchor resolves through the stable index to the same heading in that leaf; all mutable authority references use the minimum exact leaf set; historical records/dispatches retain their original bytes and meaning; review stops on the first CLEAN verdict.
