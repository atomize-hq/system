...
| `HCM-0.9` | Mechanically decompose the contract catalog without changing frozen meaning | stable `05` discovery/compatibility index; eight canonical leaf documents; exact contiguous section-to-leaf map; explicit leaf selection by future packets/dispatches; mechanical parity evidence | the ordered canonical leaf payload is byte-identical to the HCM-0.4 `05` body at `214a5b8eb182fce74478df49d4f55d226d65fdf5`; every frozen heading maps exactly once and its legacy `05` anchor links to the owning leaf; mutable guidance requires explicit leaf selection without automatic semantic routing; immutable history is unchanged; all structural, scope, and fresh-review gates pass |
...
`HCM-0.9` is a corrective control-maintenance insertion. It depends on the completed HCM-0.4 frozen contract baseline and is sequenced immediately after HCM-0.4 and before HCM-0.5. If HCM-0.9 later executes, HCM-0.5 must explicitly list its contract/evidence/verdict/gate/dock leaf authority rather than extend or default-load the monolith.
...
**Primary outputs:** the stable `05` discovery/compatibility index; the eight canonical leaf files and exact mapping frozen in [`slices/HCM-0.9/SPEC.md`](slices/HCM-0.9/SPEC.md); a slice-local mechanical decomposition verifier; updated mutable control-pack, orchestration, dispatch-template, proof, and handoff guidance for explicit leaf selection; and review/proof evidence. Existing handoff records and dispatches are inputs only and remain byte-immutable. No automatic semantic routing engine is permitted.

**Exact exit gate:** all HCM-0.9 acceptance criteria and the HCM-0.9 gate in `06-proof-and-regression-ledger.md` pass; no semantic payload byte from the frozen baseline is added, removed, reordered, or changed; every frozen `##` section has exactly one of eight leaf owners; every frozen `##`/`###` anchor resolves through the stable index to the same heading in that leaf; future packet/dispatch guidance requires explicit leaf lists; historical records/dispatches retain their original bytes and meaning; review stops on the first CLEAN verdict.
...
**Execution review-budget stop:** submit the complete execution subject at most four times and perform at most three remediation rounds. Stop immediately on CLEAN. Never run a fifth review, reuse a reviewer after remediation, self-approve, suppress or weaken valid findings, or claim execution authorization when the budget expires with unresolved Critical or Required findings.
