...
|---|---|---|---|---|---|
| Canonical artifact identities | `CanonicalArtifactKind` remains the fixed pre-membrane Markdown product projection for unmigrated sibling seams. HCM-1.3 owns the complete selected-profile universe through `ResolvedArtifactRegistry`; HCM-1.4 setup and doctor now consume one typed `ResolvedProfileDecisions`/descriptor-driven inspection closure, including custom kind and instance IDs without enum variants, filename dispatch, or compatibility translation. | `BoundaryLanded` for the selected-profile registry and setup/doctor adoption boundary; `UsefulPrecursor` for the unchanged fixed sibling projection | `handbook-engine` profile/artifact kernel | Preserve direct selected-profile ownership in setup/doctor and cut remaining layout/flow/content consumers over only in their separately reviewed slices; do not bridge or convert the fixed and selected-profile universes | kind meta-schema, profile schema, generic validation, layout/flow/content authority |
...
| Instance profile and descriptor selection | HCM-1.2 owns closed descriptors, explicit sources, bounded layering, deterministic selection, and the resolved-profile fingerprint; HCM-1.3 derives the selected registry; HCM-1.4 retains the selected condition registry and makes setup/doctor consume the same immutable profile-decision closure. Layout, flow, content authority, SDK, and other product seams remain unmigrated. | `BoundaryLanded` for the engine owner and bounded setup/doctor adoption | `handbook-engine` profile/artifact kernel | Preserve exact-source/no-ambient resolution and the shared setup/doctor closure; later slices may adopt it only through separately reviewed cutovers | kind/schema registry, project conditions, vocabulary, Context Resolution, layout/flow/content authority |
| Shipped default artifact set | HCM-1.2 packages and resolves the exact HCM-0.6 six-kind catalog plus `project_authority`, `project_context`, and conditional `environment_context` descriptors under `handbook.profile.shipped-root@1.0.0`; it packages no root Work Specification, Decision Record, or Risk Record instance. HCM-1.4 setup/doctor inspect those selected descriptors directly, write no canonical artifact, and keep the conditional instance explicitly indeterminate with no evidence fingerprint. | `BoundaryLanded` for exact package-owned data and bounded setup/doctor decision/readiness adoption; `TargetOnly` for content authority/materialization and remaining consumers | Phase 0 decision authority, then `handbook-engine` shipped profile data | Preserve the literal closure and unique constitutional root; add condition evaluation, content authority, materialization, and other consumer adoption only in separately reviewed slices | kind registry, constitutional-root contract, project condition evaluator, content authority, skill UX |
...
| Setup scaffolding | HCM-1.4 replaces setup's fixed artifact selection with typed selected-profile decisions and descriptor-driven structural inspection. It reports `author_required`/indeterminate actions, refuses rewrite before mutation, performs complete safe preflight before the retained runtime-state reset, and writes no canonical artifact. | `BoundaryLanded` for profile-aware decision/readiness adoption only; `TargetOnly` for canonical content materialization | SDK/use case over engine | Add approved content authority/materialization in its own slice; preserve direct profile ownership and add no legacy compatibility mode | profile loader, content authority, CLI UX |
| Doctor baseline | HCM-1.4 doctor exposes a closed typed profile/readiness report from the same decision/inspection closure as setup; CLI alone owns text, exact JSON rendering, and exit policy. Conditional applicability remains indeterminate and structural validity is not semantic or canonical authority. | `BoundaryLanded` for profile-aware structural machine truth only | SDK + CLI renderer | Carry the typed report into the later SDK/common response envelope without widening its authority; add evidence evaluation and semantic validation separately | artifact registry, condition evaluator, JSON protocol |
