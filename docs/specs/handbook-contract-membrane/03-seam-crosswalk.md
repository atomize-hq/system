...
| Contract-catalog topology and selective routing | `05-contracts-schemas-and-gates.md` is one 3,757-line canonical catalog; callers can name sections, but they cannot select focused canonical files and immutable historical records/dispatches bind the monolith by path or manifest | `TargetOnly` for decomposed topology and leaf routing; the frozen HCM-0.4 monolith remains canonical | Handbook Contract Membrane control pack | Execute HCM-0.9 as a zero-semantic-delta structural split: retain `05` as a stable index with every frozen anchor, move each frozen top-level section to exactly one canonical leaf, route current orchestration/dispatch/proof/handoff authority to exact leaves, and never rewrite historical records or dispatches | HCM-0.4 frozen contract baseline, control-pack map, orchestration prompt, dispatch templates, proof ledger, handoff immutability |
