...
- the live workspace posture documented in [`C-02`](../../reduced-v1/contracts/C-02-rust-workspace-and-cli-command-surface.md): `crates/engine`, `crates/flow`, and `crates/pipeline` own extracted logic directly, while `crates/compiler` is now a narrow compatibility/support seam rather than the default umbrella import surface
