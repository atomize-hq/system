...
| --- | --- | --- | --- |
| `crates/cli/src/setup.rs` | `SetupRequest`, `SetupMode`, `run_setup`, `SetupOutcome`, `SetupDisposition`, `SetupAction`, `SetupActionLabel`, `SetupRefusal`, `SetupRefusalKind` | legitimate retained narrow support seam | setup is still exposed as a compiler-owned CLI-facing support surface; this packet only records that boundary |
...
| `crates/cli/src/author.rs` | `preflight_author_charter`, `preflight_author_charter_from_input`, `author_charter_guided`, `author_charter`, `preflight_author_project_context`, `author_project_context_from_input`, `preflight_author_environment_inventory`, `author_environment_inventory`, `AuthorCharterResult`, `AuthorCharterRefusal`, `AuthorCharterRefusalKind`, `AuthorProjectContextResult`, `AuthorProjectContextRefusal`, `AuthorProjectContextRefusalKind`, `AuthorEnvironmentInventoryRefusal`, `AuthorEnvironmentInventoryRefusalKind` | legitimate retained narrow support seam | the CLI author shell still depends on compiler-owned preflight/write/refusal adapters even though some authoring core logic already lives in `handbook-engine` |
