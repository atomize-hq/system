...
fn compile_refuses_symlinked_required_artifact() {
    ...
    assert_eq!(
        err.classification,
        handbook_pipeline::pipeline_compile::PipelineCompileRefusalClassification::InvalidState
    );
    assert!(err.summary.contains("artifacts/base/BASE_CONTEXT.md"));
    ...
    assert!(
        result.documents.iter().any(|document| {
            document.path == "artifacts/project_context/PROJECT_CONTEXT.md"
                && document.status
                    == handbook_pipeline::pipeline_compile::PipelineCompileDocumentStatus::MissingOptional
        }),
